...
half4 LitPassFragment_Parallax(Varyings input): SV_Target
{
    ...
    float height = SAMPLE_TEXTURE2D(_HeightMap, sampler_HeightMap, input.uv);
        ...
        input.uv += ParallaxOcclusionMapping(input.uv, viewDirTS, saturate(dot(input.normalWS, input.viewDirWS)));

    #endif

    height = SAMPLE_TEXTURE2D(_HeightMap, sampler_HeightMap, input.uv);
    #if defined(REQUIRES_WORLD_SPACE_POS_INTERPOLATOR)
