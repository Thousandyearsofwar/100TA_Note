...
half4 LitPassFragment_Anisotopic(Varyings input) : SV_Target{
    ...
    half4 color = UniversalFragmentPBR_Anisotopic(inputData, surfaceData.albedo, surfaceData.metallic, surfaceData.specular, surfaceData.smoothness, surfaceData.occlusion, surfaceData.emission, surfaceData.alpha,T,B);
...
half4 LitPassFragment_Dither(Varyings input) : SV_Target{
    UNITY_SETUP_INSTANCE_ID(input);
    UNITY_SETUP_STEREO_EYE_INDEX_POST_VERTEX(input);
