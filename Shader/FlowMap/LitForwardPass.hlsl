...
half4 LitPassFragment_Parallax(Varyings input): SV_Target
{
    ...
    //@@@FlowMap UV Offset
    float3 flowDir = SAMPLE_TEXTURE2D(_FlowMap, sampler_FlowMap, input.uv) * 2 - 1;

    float phase0 = frac(_Time * _TimeSpeed);
    float phase1 = frac(_Time * _TimeSpeed + 0.5);
    float t = abs(phase0 * 2 - 1);
    
    float2 uv0 = input.uv - flowDir.xy * _ParrallaxDepthBias * phase0;
    float2 uv1 = input.uv - flowDir.xy * _ParrallaxDepthBias * phase1;
    SurfaceData surfaceData ;
    ZERO_INITIALIZE(SurfaceData, surfaceData);
    InitializeStandardLitSurfaceData_FlowMap(uv0, surfaceData, input.viewDirWS, input.tangentWS, input.normalWS);


    SurfaceData surfaceData1;
    ZERO_INITIALIZE(SurfaceData, surfaceData1);
    InitializeStandardLitSurfaceData_FlowMap(uv1, surfaceData1, input.viewDirWS, input.tangentWS, input.normalWS);

    lerpStandardLitSurfaceData(t, surfaceData, surfaceData1);

    //@@@Parallax UV Offset
    
    float height = SAMPLE_TEXTURE2D(_HeightMap, sampler_HeightMap, lerp(uv0, uv1, t));
    #if defined(REQUIRES_WORLD_SPACE_POS_INTERPOLATOR)
        input.positionWS = input.positionWS + input.normalWS * height * _HeightScale ;
    #endif
    #if defined(REQUIRES_VERTEX_SHADOW_COORD_INTERPOLATOR)
    ...
    inputData.normalWS = inputData.normalWS + float3(-ddx(height), ddy(height), 0) * _HeightScale ;
